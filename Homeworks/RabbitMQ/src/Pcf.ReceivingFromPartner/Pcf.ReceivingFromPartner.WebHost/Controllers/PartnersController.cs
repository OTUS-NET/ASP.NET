...
namespace Pcf.ReceivingFromPartner.WebHost.Controllers
{
                ...
                : ControllerBase
    {
            ...
            ReceivingPromoCodeRequest request)
        {
            ...
            var eventToProduce = new PromoCodeIssuedEvent
            {
            ...
            // Отправляем в очередь брокера сообщений событие о том, что промокод был выдан партнером
            await _producer.ProduceAsync(eventToProduce, "promocode.issued");
